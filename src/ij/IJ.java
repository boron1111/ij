...
public class IJ {
	...
	private static boolean isMac, isWin, isJava2, isJava14, isJava15, isJava16,
			isJava17, isJava18, isLinux, is64Bit;
	...
	/**
	 * Runs the macro contained in the string <code>macro</code>. Returns any
	 * string value returned by the macro, null if the macro does not return a
	 * value, or "[aborted]" if the macro was aborted due to an error. The
	...
	/**
	 * Runs the macro contained in the string <code>macro</code>. The optional
	 * string argument can be retrieved in the called macro using the
	 * getArgument() macro function. Returns any string value returned by the
	 * macro, null if the macro does not return a value, or "[aborted]" if the
	...
	/**
	 * Runs the specified macro or script file in the current thread. The file
	 * is assumed to be in the macros folder unless <code>name</code> is a full
	 * path. The optional string argument (<code>arg</code>) can be retrieved in
	 * the called macro or script (v1.42k or later) using the getArgument()
	 * function. Returns any string value returned by the macro, or null.
	 * Scripts always return null. The equivalent macro function is runMacro().
	 */
	...
	/** Runs the specified plugin and returns a reference to it. */
	public static Object runPlugIn(String commandName, String className,
			String arg) {
		if (arg == null)
			arg = "";
		if (IJ.debugMode)
	...
	static Object runUserPlugIn(String commandName, String className,
			String arg, boolean createNewLoader) {
		if (IJ.debugMode)
		...
		try {
			...
			if (dotIndex >= 0)
				return runUserPlugIn(commandName,
						className.substring(dotIndex + 1), arg, createNewLoader);
			if (className.indexOf('_') != -1 && !suppressPluginNotFoundError)
				error("Plugin or class not found: \"" + className + "\"\n(" + e
						+ ")");
		} catch (InstantiationException e) {
	...
	/**
	 * Converts commands that have been renamed so macros using the old names
	 * continue to work.
	 */
	private static String convert(String command) {
		if (commandTable == null) {
			...
			commandTable
					.put("Stack to HyperStack...", "Stack to Hyperstack...");
	...
	/**
	 * @deprecated replaced by IJ.log(), ResultsTable.setResult() and
	 *             TextWindow.append(). There are examples at
	...
	/**
	 * Returns the contents of the Log window or null if the Log window is not
	 * open.
	 */
	...
	public static void renameResults(String title) {
		...
		if (frame != null && (frame instanceof TextWindow)) {
			...
			if (tw.getTextPanel().getResultsTable() == null) {
				IJ.error("Rename", "\"" + tw.getTitle()
						+ "\" is not a results table");
				return;
			}
			tw.rename(title);
	...
	/**
	 * Deletes 'row1' through 'row2' of the "Results" window. Arguments must be
	 * in the range 0-Analyzer.getCounter()-1.
	 */
	 ...
	 * "Results" window if it is currently not open. Returns null if the
	 * "ImageJ" window is not open.
	 */
	 ...
	 * bar is erased if <code>currentValue&gt;=finalValue</code>. The bar is
	 * updated only if more than 90 ms have passed since the last call. Does
	 * nothing if the ImageJ window is not present.
	 */
	...
	/**
	 * Displays a message in a dialog box with the specified title. Displays
	 * HTML formatted text if 'msg' starts with "<html>". There are examples at
	 * "http://imagej.nih.gov/ij/macros/HtmlDialogDemo.txt". Writes to the Java
	 * console if ImageJ is not present.
	 */
	...
	public static void error(String title, String msg) {
		...
		if (redirectErrorMessages) {
			IJ.log(title2 + ": " + msg);
			if (abortMacro
					&& (title.contains("Open") || title.contains("Reader")))
				abortMacro = false;
		} else
	...
	/**
	 * Returns the last error message written by IJ.error() or null if there was
	 * no error since the last time this method was called.
	 * 
	 * @see #error(String)
	 */
	...
	public static String freeMemory() {
		...
		String inUseStr = inUse < 10000 * 1024 ? inUse / 1024L + "K" : inUse
				/ 1048576L + "MB";
		String maxStr = "";
		long max = maxMemory();
		if (max > 0L) {
			double percent = inUse * 100 / max;
			maxStr = " of " + max / 1048576L + "MB ("
					+ (percent < 1.0 ? "<1" : d2s(percent, 0)) + "%)";
		}
	...
	public static String time(ImagePlus imp, long startNanoTime) {
		...
		String time = seconds < 1.0 ? d2s(seconds * 1000.0, 0) + " ms" : d2s(
				seconds, 1) + " seconds";
		return time + ", " + d2s(mpixels / seconds, 1)
	...
	/**
	 * Converts a number to a rounded formatted string. The 'significantDigits'
	 * argument specifies the minimum number of significant digits, which is
	 * also the preferred number of digits behind the decimal. Fewer decimals
	 * are shown if the number would have more than 'maxDigits'. Exponential
	 * notation is used if more than 'maxDigits' would be needed.
	 */
	public static String d2s(double x, int significantDigits, int maxDigits) {
		...
		if (decimals < 0 || magnitude < significantDigits + 1 - maxDigits)
			...
			if (decimals > significantDigits)
				decimals = Math.max(significantDigits, decimals - maxDigits
						+ significantDigits);
	...
	/**
	 * Displays an error message and returns true if the ImageJ version is less
	 * than the one specified.
	 */
	public static boolean versionLessThan(String version) {
		boolean lessThan = ImageJ.VERSION.compareTo(version) < 0;
		if (lessThan)
			error("This plugin or macro requires ImageJ "
					+ version
	...
	public static int setupDialog(ImagePlus imp, int flags) {
		...
		if (stackSize > 1) {
			...
			YesNoCancelDialog d = new YesNoCancelDialog(getInstance(),
					"Process Stack?", "Process all " + stackSize + " images?"
							+ note);
			if (d.cancelPressed())
				return PlugInFilter.DONE;
			else if (d.yesPressed()) {
				if (imp.getStack().isVirtual()
						&& ((flags & PlugInFilter.NO_CHANGES) == 0)) {
					int size = (stackSize * imp.getWidth() * imp.getHeight()
							* imp.getBytesPerPixel() + 524288) / 1048576;
	...
	public static void makeRectangle(double x, double y, double width,
			double height) {
		if (width <= 0 || height < 0)
			getImage().deleteRoi();
		else
			getImage().setRoi(new Roi(x, y, width, height),
					!Interpreter.isBatchMode());
	}

	/**
	 * Creates an oval selection. Removes any existing selection if width or
	 * height are less than 1.
	 */
	...
	/**
	 * Sets the display range (minimum and maximum displayed pixel values) of
	 * the current image.
	 */
	...
	private static void setMinAndMax(ImagePlus img, double min, double max,
			int channels) {
	...
	/**
	 * Resets the minimum and maximum displayed pixel values of the current
	 * image to be the same as the min and max pixel values.
	 */
	...
	/**
	 * Resets the minimum and maximum displayed pixel values of the specified
	 * image to be the same as the min and max pixel values.
	 */
	...
	/**
	 * Sets the lower and upper threshold levels and displays the image using
	 * the specified <code>displayMode</code> ("Red", "Black & White",
	 * "Over/Under" or "No Update").
	 */
	public static void setThreshold(double lowerThreshold,
			double upperThreshold, String displayMode) {
	...
	public static void setThreshold(ImagePlus img, double lowerThreshold,
			double upperThreshold) {
		setThreshold(img, lowerThreshold, upperThreshold, "Red");
	}

	/**
	 * Sets the lower and upper threshold levels of the specified image and
	 * updates the display using the specified <code>displayMode</code> ("Red",
	 * "Black & White", "Over/Under" or "No Update").
	 */
	public static void setThreshold(ImagePlus img, double lowerThreshold,
			double upperThreshold, String displayMode) {
	...
	private static void setStackThreshold(ImagePlus imp, ImageProcessor ip,
			String method) {
	...
	public static void selectWindow(int id) {
		...
		if (imp == null)
			error("Macro Error", "Image " + id
					+ " not found or no images are open.");
		if (Interpreter.isBatchMode()) {
			...
			// timeout after 2 seconds unless current thread is event dispatch
			// thread
	...
	public static void selectWindow(String title) {
		...
		error("Macro Error", "No window with the title \"" + title
				+ "\" found.");
	}
	...
	/**
	 * Traces the boundary of the area with pixel values within 'tolerance' of
	 * the value of the pixel at the starting location. 'tolerance' is in
	 * uncalibrated units. 'mode' can be "4-connected", "8-connected" or
	...
	public static int doWand(ImagePlus img, int x, int y, double tolerance,
			String mode) {
		ImageProcessor ip = img.getProcessor();
		...
		if (w.npoints > 0) {
			...
			img.deleteRoi();
			img.setRoi(roi);
			// add/subtract this ROI to the previous one if the shift/alt key is
			// down
			if (previousRoi != null)
	 ...
	 * is "Copy", "Blend", "Average", "Difference", "Transparent",
	 * "Transparent2", "AND", "OR", "XOR", "Add", "Subtract", "Multiply", or
	 * "Divide".
	 */
	...
	/**
	 * Returns a reference to the active image, or displays an error message and
	 * aborts the plugin or macro if no images are open.
	 */
	...
	/**
	 * Returns the ImageJ version and build number as a String, for example
	 * "1.46n05", or 1.46n99 if there is no build number.
	 */
	...
	/**
	 * Returns the path to the home ("user.home"), startup, ImageJ, plugins,
	 * macros, luts, temp, current or image directory if <code>title</code> is
	 * "home", "startup", "imagej", "plugins", "macros", "luts", "temp",
	 * "current", "default", "image", otherwise, displays a dialog and returns
	 * the path to the directory selected by the user. Returns null if the
	 * specified directory is not found or the user cancels the dialog box. Also
	 ...
	 * of the supported formats, or if it is not found. With 1.41k or later,
	 * opens images specified by a URL.
	 */
	...
	/**
	 * Opens the specified file as a tiff, bmp, dicom, fits, pgm, gif or jpeg
	 ...
	 * display the image and return null.
	 */
	 ...
	 * specified file path. The path must end in ".tif", ".jpg", ".gif", ".zip",
	...
	/*
	 * Saves the specified image. The format argument must be "tiff", "jpeg",
	 * "gif", "zip", "raw", "avi", "bmp", "fits", "pgm", "png", "text image",
	 * "lut", "selection" or "xy Coordinates".
	 */
	public static void saveAs(ImagePlus imp, String format, String path) {
		...
		if (format.indexOf("tif") != -1) {
			...
			format = "LUT...";
		} else if (format.indexOf("results") != -1
				|| format.indexOf("measurements") != -1) {
			format = "Results...";
		} else if (format.indexOf("selection") != -1
				|| format.indexOf("roi") != -1) {
			path = updateExtension(path, ".roi");
			format = "Selection...";
		} else if (format.indexOf("xy") != -1
				|| format.indexOf("coordinates") != -1) {
			path = updateExtension(path, ".txt");
	...
	/**
	 * Saves the specified image in TIFF format. Displays a file save dialog if
	 * 'path' is null or an empty string. Returns 'false' if there is an error
	 * or if the user selects "Cancel" in the file save dialog.
	 */
	...
	/**
	 * Saves a string as a file. Displays a file save dialog if 'path' is null
	 * or blank. Returns an error message if there is an exception, otherwise
	 * returns null.
	 */
	 ...
	 * added to the end of the string before it is written. Returns an error
	 * message if there is an exception, otherwise returns null.
	 */
	 ...
	 */
	public static ImagePlus createImage(String title, int width, int height,
			int depth, int bitdepth) {
		return NewImage.createImage(title, width, height, depth, bitdepth,
				NewImage.FILL_BLACK);
	}

	/**
	 * Creates a new imagePlus. <code>Type</code> should contain "8-bit",
	 * "16-bit", "32-bit" or "RGB". In addition, it can contain "white", "black"
	 * or "ramp". <code>Width</code> and <code>height</code> specify the width
	 * and height of the image in pixels. <code>Depth</code> specifies the
	 * number of stack slices.
	 */
	public static ImagePlus createImage(String title, String type, int width,
	 ...
	 *            , "black" (the default), "ramp", "composite-mode",
	 ...
	 */
	public static ImagePlus createImage(String title, String type, int width,
			int height, int channels, int slices, int frames) {
		if (type.contains("label"))
			type += "ramp";
		if (!(type.contains("white") || type.contains("ramp")))
			type += "black";
		ImagePlus imp = IJ.createImage(title, type, width, height, channels
				* slices * frames);
	 ...
	 *            8, 16, 32 (float) or 24 (RGB)
	 */
	public static ImagePlus createHyperStack(String title, int width,
			int height, int channels, int slices, int frames, int bitdepth) {
		ImagePlus imp = createImage(title, width, height, channels * slices
				* frames, bitdepth);
		imp.setDimensions(channels, slices, frames);
	 ...
	 * "32-bit" or "RGB". In addition, it can contain "white", "black" or
	 * "ramp". <code>Width</code> and <code>height</code> specify the width and
	 * height of the image in pixels. <code>Depth</code> specifies the number of
	 * stack slices.
	 */
	public static void newImage(String title, String type, int width,
			int height, int depth) {
	...
	/**
	 * Returns true if the <code>Esc</code> key was pressed since the last
	 * ImageJ command started to execute or since resetEscape() was called.
	 */
	...
	/**
	 * This method sets the <code>Esc</code> key to the "up" position. The
	 * Executer class calls this method when it runs an ImageJ command in a
	 * separate thread.
	 */
	...
	/**
	 * Causes IJ.error() output to be temporarily redirected to the "Log"
	 * window.
	 */
	...
	/**
	 * Set 'true' and IJ.error() output will be temporarily redirected to the
	 * "Log" window.
	 */
	...
	/**
	 * Returns the class loader ImageJ uses to run plugins or the system class
	 * loader if Menus.getPlugInsPath() returns null.
	 */
	...
	/**
	 * Returns, as an array of strings, a list of the LUTs in the Image/Lookup
	 * Tables menu.
	 */
	...
	/**
	 * Displays a stack trace. Use the setExceptionHandler method() to override
	...
	/**
	 * Installs a custom exception handler that overrides the handleException()
	 * method.
	 */
	...
	public static void notifyEventListeners(int eventID) {
		synchronized (eventListeners) {
			for (int i = 0; i < eventListeners.size(); i++) {
				IJEventListener listener = (IJEventListener) eventListeners
						.elementAt(i);
